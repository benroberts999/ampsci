...
TEST_CASE("Physics: Radiative Potential", "[RadPot][QED][integration]") {
  ...
  // "Regresseion" tests: Self-energy (parts):
  {
    ...
    wf.solve_core("Hartree", "[Xe]", std::nullopt, 0.0, false);
...
helper::compare_QED(const std::vector<QEDData> &QEDdata,
  ...
  for (const auto &[atom, data] : QEDdata) {
    ...
    wf0.solve_core("HartreeFock", atom.core, std::nullopt, 0.0, false);
    ...
    wf.set_HF(HF::Method::HartreeFock, atom.core, std::nullopt, 0.0, false);
