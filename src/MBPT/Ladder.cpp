...
double Lkmnab(int k, const DiracSpinor &m, const DiracSpinor &n,
  ...
  return L1(k, m, n, a, b, qk, excited, SJ, Lk) +
         L23(k, m, n, a, b, qk, core, excited, SJ, Lk);
}
...
#pragma omp parallel for reduction(+ : l1)
  for (auto ir = 0ul; ir < excited.size(); ++ir) {
    ...
    for (const auto &s : excited) {
      ...
      const auto inv_de = 1.0 / (a.en() + b.en() - r.en() - s.en());
      ...
      for (auto u = u0; u <= uI; u += 2) {
        ...
        for (auto l = l0; l <= lI; l += 2) {
          ...
          const auto sj_r = SJ->get(m, a, k, l, u, r);
          const auto sj_s = SJ->get(n, b, k, l, u, s);

          const auto Q_lrsab = qk.Q(l, r, s, a, b);
          const auto L_lrsab = Lk ? Lk->Q(l, r, s, a, b) : 0.0;
...
#pragma omp parallel for reduction(+ : l23)
  for (auto ir = 0ul; ir < excited.size(); ir++) {
    ...
    for (const auto &c : core) {
      ...
      const auto inv_de_acmr = 1.0 / (a.en() + c.en() - m.en() - r.en());

      const auto P_kcnrb = qk.P(k, c, n, r, b);
      if (P_kcnrb != 0.0) {
        const auto Lambda_kmrac = Lk ? Lk->P(k, m, r, a, c, SJ) : 0.0;
        const auto P_kmrac = qk.P(k, m, r, a, c, SJ);
      ...
      const auto P_kcmra = qk.P(k, c, m, r, a);
      if (P_kcmra != 0.0) {
