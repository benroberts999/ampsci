  ...
  for (auto r_index = 0ul; r_index < excited.size(); ++r_index) {
    ...
    for (const auto &s : excited) {
      ...
      for (auto u = u0; u <= uI; u += 2) {
        const auto Q_umnrs = global_fk.scr(u) * qk.Q(u, m, n, r, s);
        if (Q_umnrs == 0.0)
        ...
        for (auto l = l0; l <= lI; l += 2) {
          ...
          const auto Q_lrsab = global_fk.scr(l) * qk.Q(l, r, s, i, b);
...
#pragma omp parallel for reduction(+ : l23)
  for (auto r_index = 0ul; r_index < excited.size(); ++r_index) {
    ...
    for (const auto &c : core) {
      ...
      const auto P_kcnrb = qk.P2(k, c, n, r, b, *SJ, global_fk.fk);
      if (P_kcnrb != 0.0) {
        const auto P_kmrac = qk.P2(k, m, r, i, c, *SJ, global_fk.fk);
      ...
      const auto P_kcmra = qk.P2(k, c, m, r, i, *SJ, global_fk.fk);
      if (P_kcmra != 0.0) {
        const auto P_knrbc = qk.P2(k, n, r, b, c, *SJ, global_fk.fk);
