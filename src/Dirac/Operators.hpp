...
public:
  RadialHamiltonian(const Grid &rgrid, const int in_z, const double in_alpha)
      : m_gr(&rgrid), m_z(in_z), m_alpha(in_alpha), m_c(1.0 / m_alpha),
        m_c2(m_c * m_c), m_Vk({}), m_v_mag({}) {}
  ...
  // d_r F = (c00, c01)F
  //         (c10, c11)
  double derivMat_c00(int k, double, std::size_t i) const {
    auto h = m_v_mag.empty() ? 0.0 : m_v_mag[i] * m_gr->drdu[i];
    return (double(-k)) * m_gr->drduor[i] + h;
  }
  double derivMat_c01(int k, double en, std::size_t i) const {
    const auto &v = get_v(k);
    return m_alpha * (en + 2.0 * m_c2 - v[i]) * m_gr->drdu[i];
  }
  double derivMat_c10(int k, double en, std::size_t i) const {
    const auto &v = get_v(k);
    return -m_alpha * (en - v[i]) * m_gr->drdu[i];
  }
  double derivMat_c12(int k, double, std::size_t i) const {
    auto h = m_v_mag.empty() ? 0.0 : m_v_mag[i] * m_gr->drdu[i];
    return double(k) * m_gr->drduor[i] - h;
  }
