...
/*!
  @brief Exchange of a single electron in orbital Fa, acting on Fv: X_a Fv.
  @details
  ...
  - The b=Fa term of vexFa(), scaled to ONE electron (weight 1/[j_a] rather
    than the subshell occupancy): the spherically-averaged one-electron
    self-exchange.
  - Used for the V^{N-1} (residual ion) potential: removing one electron from
    ...
    from the exchange, V^{N-1}_a = V_HF - (y^0_aa + X_a).
