...
// Solves:  (H - e - w)X = -h*psi for X
{
  ...
  if (dF20 == 0) {
    DiracODE::solve_inhomog(dF, phi0.en + omega, vl, H_mag, alpha, -1 * hphi0);
  } else {
    const auto vx0 = form_approx_vex_any(dF, core);
    const auto v0 = NumCalc::add_vectors(vl, vx0);
    const auto rhs0 = (vx0 * dF) - vex_psia_any(dF, core) - hphi0;
    DiracODE::solve_inhomog(dF, phi0.en + omega, v0, H_mag, alpha, rhs0);
    // const auto a = 0.0;
    // const auto l = (1.0 - a);
    // dF = l * dF + a * dF0;
  }
