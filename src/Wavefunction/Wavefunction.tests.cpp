...
TEST_CASE("Wavefunction", "[wf][unit]") {
  ...
  wf.solve_core("HartreeFock", "[Ne]", std::nullopt, 1.0e-4);
  ...
  wfQED.set_HF(HF::Method::HartreeFock, "[Ne]", std::nullopt, 1.0e-4);
  ...
  Wavefunction wfBr({num_points, r0, rmax, b, "loglinear", -1.0},
                    {"Na", -1, "Fermi", -1.0, -1.0}, 1.0);
  wfBr.solve_core("HartreeFock", "[Ne]", HF::Breit::Params{1.0}, 1.0e-4);
  ...
  hlike.solve_core("HartreeFock", "[]"); // should do nothing!
  ...
  // hlike_nr.solve_core("HartreeFock", "[]");
...
TEST_CASE("Wavefunction - continuum", "[wf][cntm][unit]") {
...

  {
    ...
    Wavefunction wf({num_points, r0, rmax, b, "loglinear", -1.0},
                    {"Ne", -1, "Fermi", -1.0, -1.0}, 1.0);
    wf.solve_core("ApproxHF", "[Ne]", std::nullopt, 1.0e-8);
...

  {
    ...
    Wavefunction wf({num_points, r0, rmax, b, "loglinear", -1.0},
                    {"Ne", -1, "Fermi", -1.0, -1.0}, 1.0);
    wf.solve_core("KohnSham", "[Ne]", std::nullopt, 1.0e-8);
