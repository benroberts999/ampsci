...
void solveMixedState_cntm(DiracSpinor &dF, const DiracSpinor &Fa,
  ...
  // Gram matrix <r_i|r_j> of the stored residuals (as rows), updated
  // incrementally: only the new row/column costs spinor dots each iteration
  // (the stored entries never change); dropping the oldest history entry
  // just deletes the first row and column.
  std::vector<std::vector<double>> rr_hist;
  const auto drop_oldest = [&]() {
    g_hist.erase(g_hist.begin());
    r_hist.erase(r_hist.begin());
    rr_hist.erase(rr_hist.begin());
    for (auto &row : rr_hist) {
      row.erase(row.begin());
    }
  };
  ...
  for (; its < max_its; its++) {
    ...
    r_hist.push_back(r);
    {
      std::vector<double> row(r_hist.size());
      for (std::size_t i = 0; i < r_hist.size(); ++i) {
        row[i] = r_hist[i] * r_hist.back();
      }
      for (std::size_t i = 0; i < rr_hist.size(); ++i) {
        rr_hist[i].push_back(row[i]);
      }
      rr_hist.push_back(std::move(row));
    }
...
void solveContinuumMixedState(DiracSpinor *phi, DiracSpinor *Freg,
  ...
  // F_irr at en_+ in the (fixed) local potential v. Reuse if the caller
  // already holds the pair at this energy (e.g. cached across the outer TDHF
  // iterations at fixed omega).
  ...
  // Records the K-matrix amplitude of the latest solve.
  double K_last = 0.0;
  ...
  // Occupied-component convention, matching the bound solver: project out
  // ONLY the diagonal Fa component (norm conservation), and only when the
  // channel shares Fa's kappa (even-parity operators; never for E1). All
  // OTHER occupied components are kept: their dV contributions cancel only
  // PAIRWISE across channels (<b|X_a> against <a|X_b>), so removing them
  // here while the bound channels keep them breaks the cancellation -- and
  // gauge invariance, since the removed amplitudes are off-shell
  // (gauge-dependent). No occupied state is near-resonant at en_+ > 0, so
  // none of the bound solver's projection/restoration machinery is needed.
  // (Continuum normalisation unaffected: overlaps with decaying bound
  // orbitals are finite.)
  ...
  // Iterate the nonlocal exchange:
  //   (h_r^{(v)} - en_+) phi = Ux*phi - V^exch*phi - Fs,  v = vl + Ux,
  // with Anderson/Pulay (DIIS) mixing, exactly as the bound
  // solveMixedState_cntm(): the equation is linear, and at low en_+ (just
  // above a threshold, especially behind a centrifugal barrier) the damped
  // Picard iteration's multiplier can exceed 1 and diverge; Anderson
  // converges on the true operator's conditioning. K_phi tracks the
  // standing-wave amplitude of the CURRENT phi (K is linear in phi, so it
  // mixes with the same coefficients; the core orthogonalisation does not
  // change the asymptotic amplitude).
  const auto G = [&](const DiracSpinor &x) {
    auto vnl = HF::vexFa(x, core);
  ...
  for (; its < max_its; ++its) {
    ...
    // B_ij = <r_i|r_j>. Ill-conditioned (saturated) history: drop oldest.
    LinAlg::Vector<double> c;
    while (true) {
      LinAlg::Matrix<double> B(m + 1, m + 1);
      LinAlg::Vector<double> bvec(m + 1);
    ...
    // New iterate: phi = sum_i c_i g_i (DIIS extrapolation); K follows with
