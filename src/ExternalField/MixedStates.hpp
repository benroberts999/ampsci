...
/*!
  ...
  If @p Freg already holds the homogeneous pair at en_+ (Freg->en() == en_+
  and nonzero), the (expensive) homogeneous builds are skipped and the given
  F_reg/F_irr are used; this also removes the per-rebuild renormalisation
  noise when iterating at fixed omega. Otherwise they are built and written.

  @param phi    In/out: continuum correction orbital. Used as the starting
                guess if nonzero (faster at a nearby omega).
  @param Freg   In/out: regular energy-normalised continuum orbital at en_+.
  @param Firr   In/out: irregular partner.
  @param K      Output (if non-null): standing-wave (K-matrix) amplitude of
                phi from the final iteration, phi -> K * F_irr at large r.
  @param Fa     Bound orbital phi_a (provides en_a and the grid).
  @param omega  External-field frequency (en_+ = Fa.en() + omega must be > 0).
  ...
  @note If @p core is empty, reduces to a single bare local continuum (forward)
        solve.
  @warning Requires en_+ > 0 and a grid dense enough at large r. When reusing
           @p Freg / @p Firr, they must have been built in the SAME local
           potential (vl + KS exchange) -- this is not checked.
*/
void solveContinuumMixedState(DiracSpinor *phi, DiracSpinor *Freg,
