...
public:
  ...
  */
  ...
  //! If true, eps is the relative change of dPsi (sqrt of the ratio); if false
  //! (default), the squared ratio. See eps_dPsi().
  //! @note Enabling sqrt rescales eps, so eps_target() may need retuning.
  void set_eps_sqrt(bool use_sqrt) { m_eps_sqrt = use_sqrt; }
