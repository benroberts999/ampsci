...
TEST_CASE("InputBlockLegacy", "[InputBlock][unit]") {
  ...
  InputBlockLegacy ib("name1", {{"k1", "1"}, {"k2", "2.5"}});
  ib.add(Option{"k3", "number_3"});
  ib.add(Option{"k4", ""});
  ib.add(InputBlockLegacy("blockA", {{"kA1", "old_val"}, {"kA1", "new_val"}}));
  ib.add(InputBlockLegacy("blockB", {{"keyB1", "valB"}}));
  // 'true' means will be merged with existing block (if exists)
  ib.add(InputBlockLegacy("blockB", {{"keyB2", "17.3"}}), true);
  ...
  // Construct a new InputBlockLegacy using the string output of another
  std::stringstream ostr1;
  ib.print(ostr1);
  InputBlockLegacy ib2("name2", ostr1.str());
...
//==============================================================================
void run_tests(const IO::InputBlockLegacy &ib) {
