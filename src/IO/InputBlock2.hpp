...
public:
  ...
  explicit InputBlock2(std::string name,
  ...
  */
  static InputBlock2 from_file(const std::string &path) {
    std::ifstream f(path);
    if (!f)
      throw std::runtime_error("InputBlock2: cannot open file: " + path);
    try {
      // ignore_comments = true: strips // and /* */ before parsing
      auto j = nlohmann::json::parse(f, nullptr,
                                     /*allow_exceptions=*/true,
                                     /*ignore_comments=*/true);
      return InputBlock2{path, std::move(j)};
    } catch (const nlohmann::json::parse_error &e) {
      throw std::runtime_error(std::string("InputBlock2: JSON parse error in ") +
                               path + ": " + e.what());
    }
  }
  ...
  */
  std::optional<InputBlock2> getBlock(std::string_view name) const {
    const auto it = m_node.find(std::string(name));
    if (it == m_node.end() || !it->is_object())
      return std::nullopt;
    return InputBlock2{std::string(name), *it};
  }

  //! Returns the child block named @p name, or an empty block if not found.
  InputBlock2 get_block(std::string_view name) const {
    auto tmp = getBlock(name);
    return tmp ? *tmp : InputBlock2{std::string(name)};
  }
  ...
  */
  ...
  void set_block(std::string_view name, InputBlock2 block) {
...
//==============================================================================
// InputBlock2::get<T> implementation
//==============================================================================

template <typename T>
std::optional<T> InputBlock2::get(std::string_view key) const {
...
//==============================================================================
// InputBlock2::check implementation
//==============================================================================

inline bool
InputBlock2::check(const std::vector<std::pair<std::string, std::string>> &list,
                   bool print) const {
  bool all_ok = true;
