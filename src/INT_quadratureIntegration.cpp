...
int nquad14 = 14;
double c14[14] = {1360737653653,   9821965479386,   -7321658717812,
...
*/
{
  ...
  // Note: normally, NGP is so large, that there is no difference between
  // nquad=1 to 14!
...
int diff(const std::vector<double> &f, const std::vector<double> &drdt,
  ...
  // Problem: This calcs the derivative for a few (4) points AFTER pinf; where
  // it oscilates. Means the first 0 of dP won't correspond to first max of P!
  ...
  */
  ...
  deriv[NGP - 3] = (f[NGP - 5] - 8 * f[NGP - 4] + 8 * f[NGP - 2] - f[NGP - 1]) /
                   (12 * h * drdt[NGP - 3]);

  deriv[3] =
      (-1 * f[0] + 9 * f[1] - 45 * f[2] + 45 * f[4] - 9 * f[5] + 1 * f[6]) /
      (60 * h * drdt[3]);
  deriv[NGP - 4] = (-1 * f[NGP - 7] + 9 * f[NGP - 6] - 45 * f[NGP - 5] +
                    45 * f[NGP - 3] - 9 * f[NGP - 2] + 1 * f[NGP - 1]) /
                   (60 * h * drdt[NGP - 4]);

  for (auto i = 4u; i < (NGP - 4); i++) {
    //		deriv[i] = ( (1/56)*f[i-4] - (4/21)*f[i-3] + 1*f[i-2] - 4*f[i-1]
    //		- (1/56)*f[i+4] + (4/21)*f[i+3] - 1*f[i+2] + 4*f[i+1] ) /
    //(5*h*drdt(i));
    deriv[i] = ((1. / 8) * f[i - 4] - (4. / 3) * f[i - 3] + 7 * f[i - 2] -
...
//******************************************************************
double integrate1(const std::vector<double> &f1, double h, int l, int m,
...
*/
{
  ...
  if ((a_start == 0 && a_end == 0) || ((m - l) <= 2 * nquad14))
    return a * h;
...
double integrate2(const std::vector<double> &f1, const std::vector<double> &f2,
                  double h, int l, int m, int a_start, int a_end)
/*
Integrates f1*f2*f3
NOTE: f3 should be wronskian!Jacobian?Whatever
*/
{
  ...
  for (int i = 0; i < nquad14; i++) {
    a +=
        (c14[i] / dd14) * (a_start * f1[ll] * f2[ll] + a_end * f1[mm] * f2[mm]);
    ll++;
    mm--;
  }
...
//******************************************************************
double integrate3(const std::vector<double> &f1, const std::vector<double> &f2,
                  const std::vector<double> &f3, double h, int l, int m,
                  int a_start, int a_end)
/*
Integrates f1*f2*f3
NOTE: f3 should be wronskian!Jacobian?Whatever
...
//******************************************************************
double integrate4(const std::vector<double> &f1, const std::vector<double> &f2,
