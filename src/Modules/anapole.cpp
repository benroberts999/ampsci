//Module for computing pnc amplitude induced by spin dependent nuclear anapole moment 
...
double pnc_sd(std::vector<Coulomb::meTable<double>> ME_tables,
          const std::vector<DiracSpinor> &spectrum, const DiracSpinor &w,
...
void anapole(const IO::InputBlock &input, const Wavefunction &wf) {
  ...
  //NSD weak interaction
  DiracOperator::PNCsd hpnc(c, t, wf.grid());
  ...
  if (rpaQ) {
    ...
    double  pnc = pnc_sd(Table, spectrum, Fw, Fv, I2, Fv2, Fw2);
    ...
    std::vector<Coulomb::meTable<double>> Table =
        compute_me_3f(&hpnc, &he1, spectrum, Fw, Fv);
    std::cout << "Complete. \n";
...
// namespace Module
namespace {
  const Register r_anapole{
    "anapole", "SOS pnc spin dependent amplitude", &anapole};
  }// namespace
