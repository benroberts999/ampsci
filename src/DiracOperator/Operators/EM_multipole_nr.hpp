...
public:
  E5_nr(const Grid &gr, int K, double omega)
      : TensorOperator(K, Angular::evenQ(K) ? Parity::odd : Parity::even, 1.0, gr.r(), 0, Realness::real, true), m_K(K) {
        updateFrequency(omega);
  }
  ...
  double radialIntegral(const DiracSpinor &Fa,
    ...
    if (m_K == 1){
      ...
      // Using orthogonality Rab(+1) = 0, Rab(-1) = -2Gab
      ...
      if (m_K == 2){
        const auto cx2 = (1.0 / 15.0)*std::sqrt(2.0 / 3.0);
        const auto dk = double(Fa.kappa() + Fb.kappa());

        return cx2*(dk*m_q*Rab(-1, m_vec, Fa, Fb) - 2*m_q*Rab(+1, m_vec, Fa, Fb));
      }
...
public:
  S5_nr(const Grid &gr, int K, double omega)
      : TensorOperator(K, Parity::odd, 1.0,
                       gr.r(), 0, Realness::real, true),
        m_K(K) {
    if (omega != 0.0)
      updateFrequency(omega);
...
V5_sigma_nr(const Grid &grid, int k, int sigma) {
  ...
  case +1:
    return std::make_unique<E5_nr>(grid, k, 1.0e-4);
