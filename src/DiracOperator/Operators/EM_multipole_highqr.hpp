...
class VEk_highq final : public EM_multipole {
public:
  VEk_highq(const Grid &gr, int K, double)
    : EM_multipole(K, Parity::odd, -std::sqrt(2.0) / 3.0, {},
                   Realness::imaginary, false, &gr, 'V', 'E', false, true) {} // low_q = false, high_q = true
...
//! High qr form of Vector magnetic. Only for K=1 (zero otherwise)
class VMk_highq final : public EM_multipole {
public:
  VMk_highq(const Grid &gr, int K, double omega)
    : EM_multipole(K, Parity::even, 0, gr.r(), Realness::imaginary, true, &gr,
                   'V', 'M', false, true) {
    updateFrequency(omega);
  }

  //! @note VMk_highq angular factor has an extra (ka+kb) prefactor vs. the base.
...
class VLk_highq final : public EM_multipole {
public:
  VLk_highq(const Grid &gr, int K, double)
...
public:
  Sk_highq(const Grid &gr, int K, double omega)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::even : Parity::odd, 1.0,
                   gr.r(), Realness::real, true, &gr, 'S', 'T', false, true) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
  DiracSpinor radial_rhs(const int kappa_a,
...
class AEk_highq final : public EM_multipole {
public:
  AEk_highq(const Grid &gr, int K, double omega)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::odd : Parity::even, 1.0,
                   gr.r(), Realness::real, false, &gr, 'A', 'E', false, true) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
  DiracSpinor radial_rhs(const int kappa_a,
...
class ALk_highq final : public EM_multipole {
public:
  ALk_highq(const Grid &gr, int K, double omega)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::odd : Parity::even, 1.0,
                   gr.r(), Realness::real, true, &gr, 'A', 'L', false, true) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
  DiracSpinor radial_rhs(const int kappa_a,
...
class AMk_highq final : public EM_multipole {
public:
  AMk_highq(const Grid &gr, int K, double omega)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::even : Parity::odd, 1.0,
                   gr.r(), Realness::real, true, &gr, 'A', 'M', false, true) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
...
class Phi5k_highq final : public EM_multipole {
public:
  Phi5k_highq(const Grid &gr, int K, double omega,
             double alpha = PhysConst::alpha)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::odd : Parity::even, 1.0,
                   gr.r(), Realness::real, true, &gr, 'A', 'T', false, true),
      m_alpha(alpha) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
...
//! NOTE: If K=0, omega should be (ea-eb); for K=1 should be q = alpha*omega!
class S5k_highq final : public EM_multipole {
public:
  S5k_highq(const Grid &gr, int K, double omega, double alpha = PhysConst::alpha)
    : EM_multipole(K, Angular::evenQ(K) ? Parity::odd : Parity::even, 1.0,
                   gr.r(), Realness::real, true, &gr, 'P', 'T', false, true),
      m_alpha(alpha) {
    if (omega != 0.0)
      updateFrequency(omega);
  }
