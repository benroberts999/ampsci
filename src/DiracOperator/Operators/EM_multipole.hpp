...
- in terms of j(k*r), where k = alpha*omega
- If transition_form is true, will use the "t" (transition form); if false, will use the "moment" form. Nb: E1, E2 etc. is the "moment" form, but alpha exp(iqr) goes to transition form
*/
...
public:
  ...
  std::string name() const override final { return "Ek_omega"; }
  ...
  void updateFrequency(const double omega) override final {
    const auto k = std::abs(m_alpha * omega);

    // should be 1 for "transition" operator, otherwise factor for the "moment" form
    m_constant = m_transition_form ?
                     1.0 :
                     qip::double_factorial(2 * m_K + 1) / qip::pow(k, m_K);

    SphericalBessel::fillBesselVec_kr(m_K, k, m_vec, &j1);
    SphericalBessel::fillBesselVec_kr(m_K + 1, k, m_vec, &j2);
  }
