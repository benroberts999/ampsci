...
public:
  ...
  double radialIntegral(const DiracSpinor &Fa,
    ...
    return - (1.0/3.0) * (dk * Rab(-1, ones, Fa, Fb) - Rab(+1, ones, Fa, Fb)); // sigma = -1
  }
...
inline std::unique_ptr<DiracOperator::TensorOperator>
V_sigma_nr(const Grid &grid, int k, int sigma) {

  switch (sigma) {
  
  case +1:
    std::make_unique<E5k_w_nr>(grid, 1, 1.0e-4);

  case -1:
    std::make_unique<L5k_w_nr>(grid, 1, 1.0e-4);

  case 0:
    std::make_unique<M5k_w_nr>(grid, 1, 1.0e-4);
