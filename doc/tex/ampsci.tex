...
\def\updatedDate{1 October 2020}
...
The method we use to find the self-consistent potential is the relativistic Hartree-Fock (HF) method, which includes the electron exchange interaction.
...
We use this only as a starting point for the HF procedure, so the the final result does not depend on this potential. 
...
This is very numerically unstable whenever $F_a^\dag F_a$ is small.
However, this is not a major problem, since when $F_a$ is small, the exchange potential is less important (the combination $V_xF_a$ enters the equations).
...
In this case, a complete set of orbitals can be approximately expanded in terms of a finite number of discreet states, which includes the $\en>0$ `continuum' states.
...
In the previous section, I described the usual second-order many-body perturbation theory method using the standard Goldstone diagram technique.
...
\label{eq:GreenInt2}
\end{align}
% (this time it is the other two terms which survive).
...
%\footnote{This is equivalent (matrices are symmetric).}:
\be\label{eq:PiGreen}
...
%\begin{align}
%\delta\en_v^{\rm dir}&=\bra{v}\Sigma^{(2)}_{\rm d}\ket{v}
...
%\begin{align}
%\delta\en_v^{\rm exch}&=\bra{v}\Sigma^{(2)}_{\rm x}\ket{v}
...
%
%%\begin{multline}
%%\Sigma_{12,\rm{x}}^{(\k_v)} = i\int\frac{\d\w}{2\pi} \sum_{k\lambda}\frac{(-1)^{k+\lambda}}{[j_v]}
%%\sum_a^{\rm core}\sum_{\k}^{\a,\b}  
%%C^k_{v\a}C^k_{a\b}  \times \\
%%\sum_{ij} g_{1i}^\a(\en + \w) \, \hat q^k_{1j}  \Bigg[
%%C^\lambda_{v\b}C^\lambda_{a\a}
%%\sixj{j_v}{j_\a}{k}{j_a}{j_\b}{\lambda} \, p^a_{ij} \, g^{{\rm ex},\b}_{j2}(\en_a-\w) \\
%%+ 
%%(-1)^{j_a-j_\b}C^\lambda_{va}C^\lambda_{\beta\a}\sixj{j_v}{j_\a}{k}{j_\b}{j_a}{\lambda} \,
%% g^{{\rm ex},\b}_{ij}(\en_a+\w) \, p^a_{j2} 
%%\Bigg]
%%\hat q^\lambda_{i2},
%%\end{multline}
%
...
%\notag\\
%&=\left[g(\en+\w_r)^{-1} + i\w_i \mathbb{I}\right]^{-1}
\end{align}
This $g(\en+\w)$ will be a complex-valued matrix; $g(\en+\w_r)$ is real-valued.
The integration over $\w_i$ can then proceed using standard numerical integration. % (we use a logarithmic grid for the $\w_i$ points).
...
%, which arises due to the modification of the (direct) Hartree-Fock potential for the excited core electron in the polarisation loop (i.e., the excited core particle in the polarisation loop moves in the field of $N-2$ other core electrons instead of the usual $V^{N-1}$ potential).
...
\end{align}
This is called the screened Coulomb operator, and includes electron screening to all-orders.
...
%All-orders correlation potential in the screened Coulomb interaction.
%The direct term could equivalently have been written in terms of the screened polarisation operator:
%\be
%\widetilde \Pi(\w) = \hat \Pi\left[1-\hat Q\hat \Pi(\w)\right]^{-1},
%\ee
%which is essentially the series of diagrams presented in Fig.~\ref{fig:Screening}, starting from the second term (and the polarisation operator does not include the photon legs).
...
%However, we note that while the coupled-cluster method includes {\em all} double
...
%%--------------------------------------------------------
%\subsection{Angular reduction for screened Coulomb}
%
%The screened Coulomb (and polarisation) matrices involve products of $\Pi \hat Q$.
%These include their own summation over angular momentum states (and angular integrals), and the angular reduction must be done on top of that done in Eq.~(\ref{eq:RadialSigmaDir}).
%Using Eq.~(\ref{eq:RadialPolarisation}), this can be done simply:
%\begin{multline}
%\hat \Pi(\w) \hat Q  %= \sum_k & \pi^k q^k
% =    i\sum_k\sum_{a}^{\rm core}\sum_{n}^{(\kappa)} \frac{(C^k_{an})^2}{[k]}\\
%%~~~~
%\times \sum_i p^a_{1i}
%\Big[g_{1i}^{n}(\en_{a}-\w)
%+g_{1i}^{n}(\en_{a}+\w)
%\Big] \, q^k_{i2},
%\end{multline}
%where $g^{n}$ is the excited-states-only Green's function with $\kappa=\k_n$.
%\note{This needs to be checked, not 100\% sure!}
%This matrix can now be inserted directly into Eq.~(\ref{eq:ScreenQ}) or (\ref{eq:ScreenPi}) to perform the screening.
%Note that this matrix depends only of $\w$.
%\note{include 1/[k]????}
...
%Therefore, we require a single potential that simultaneously describes the occupied core and excited states, 
...
%\begin{figure*}[h]%[h!]
%\centering
%\includegraphics[height=0.036\textwidth]{img/Sigma/CoulombAll}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/equals}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/CoulombScreening0}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/plus}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/CoulombScreening_hp}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/plus}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/CoulombScreening_hp_2}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/plus}~~
%\includegraphics[height=0.036\textwidth]{img/Sigma/dotdotdot}
...
The interplay between the Breit/QED and correlation corrections turns out to be important~\cite{Derevianko2001,GingesQED2015,Ginges2016}, so for high-accuracy calculations, the Breit and QED effects must be included together including correlations.
...
$\delta$Scr. is the all-orders screening correction (beyond second-order), and $\delta$h-p is the hole-particle interaction.
$\Delta = E_{\rm Theory}-E_{\rm Expt.}$, $\epsilon = \Delta/E_{\rm Expt.}$.
Note that for the $n=7$ states (*) the correlation potential was evaluated at the energies of the $n=6$ states; the agreement with experiment is improved if these are calculated at their own energies (see discussion above).
...
\hline~\\[-0.3cm]
$6s_{1/2}$ & 27954.1 & 4455.6  & -1656.7   & 701.0 & -4.6  & -21.7 & 31427.8 & 31406.5 & 21.3  & 0.07\%  \\
$6p_{1/2}$ & 18790.5 & 1746.7  & -600.0    & 348.4 & -11.2 & 1.1   & 20275.5 & 20228.2 & 47.3  & 0.2\%   \\
$6p_{3/2}$ & 18388.8 & 1549.9  & -529.0    & 308.2 & -4.3  & 0.1   & 19713.7 & 19674.2 & 39.5  & 0.2\%   \\
$5d_{3/2}$ & 14138.5 & 3424.1  & -1107.7   & 693.7 & 20.5  & 5.5   & 17174.5 & 16907.2 & 267.3 & 1.6\%   \\
$5d_{5/2}$ & 14162.6 & 3239.8  & -1024.4   & 656.6 & 24.1  & 4.8   & 17063.5 & 16809.6 & 253.8 & 1.5\%   \\
*$7s_{1/2}$ & 12112.2 & 910.2   & -315.2    & 121.2 & -1.3  & -5.0  & 12822.1 & 12870.9 & -48.8 & -0.4\%  \\
*$7p_{1/2}$ & 9222.6  & 487.1   & -157.2    & 87.9  & -3.6  & 0.4   & 9637.2  & 9641.1  & -3.9  & -0.04\% \\
*$7p_{3/2}$ & 9079.2  & 441.1   & -141.9    & 79.9  & -1.4  & 0.04  & 9456.9  & 9460.1  & -3.1  & -0.03\% \\
\hline
\hline
\end{tabular}
\end{table*}
...
$\delta\,\lambda\Sigma$ is the contribution from scaling $\Sigma\to\lambda\Sigma$, where $\lambda$ is chosen to reproduce the lowest valence experimental energies ($\lambda\approx1$).
Note that for the $n=7$ states (*) the correlation potential was evaluated at the energies of the $n=6$ states.
(Units: $a_0$)
%; the agreement with experiment is improved if these are calculated at their own energies (see discussion above).
...
& \multicolumn{1}{c}{$\epsilon$}         \\
\hline
$6p_{1/2}$ & $6s_{1/2}$ & -5.278  & 0.303  & 0.587  & -0.192 & 0.074  & 0.000  & -0.003 & -0.002 & -4.511  & -4.5057(16) & 0.006  & -0.12\% \\
$6p_{3/2}$ & $6s_{1/2}$ & 7.426   & -0.413 & -0.843 & 0.277  & -0.106 & 0.000  & 0.005  & 0.002  & 6.350   & 6.3398(22)  & -0.010 & -0.16\% \\
*$7p_{1/2}$ & $6s_{1/2}$ & -0.372  & 0.134  & -0.061 & 0.027  & 0.007  & -0.002 & 0.002  & -0.008 & -0.272  & -0.2781(4)  & -0.006 & 2.2\%   \\
*$7p_{3/2}$ & $6s_{1/2}$ & 0.695   & -0.186 & 0.096  & -0.039 & -0.007 & 0.000  & -0.003 & 0.009  & 0.565   & 0.5742(1)   & 0.009  & 1.5\%   \\[0.1cm]
*$6p_{1/2}$ & $7s_{1/2}$ & 4.413   & 0.037  & -0.225 & 0.061  & -0.077 & 0.006  & -0.004 & 0.020  & 4.230   & 4.249(4)    & 0.019  & 0.46\%  \\
*$6p_{3/2}$ & $7s_{1/2}$ & -6.671  & -0.042 & 0.242  & -0.062 & 0.096  & -0.002 & 0.005  & -0.025 & -6.459  & -6.489(5)   & -0.030 & -0.46\% \\
*$7p_{1/2}$ & $7s_{1/2}$ & -11.009 & 0.088  & 0.743  & -0.251 & 0.067  & 0.002  & -0.007 & 0.006  & -10.361 & -10.325(5)  & 0.036  & -0.35\% \\
*$7p_{3/2}$ & $7s_{1/2}$ & 15.345  & -0.117 & -1.104 & 0.374  & -0.099 & 0.000  & 0.010  & -0.009 & 14.400  & 14.344(7)   & -0.056 & -0.39\% \\[0.1cm]
$6p_{1/2}$ & $5d_{3/2}$ & 8.978   & -0.340 & -1.892 & 0.490  & -0.312 & -0.009 & -0.003 & 0.114  & 7.027   &             &        &         \\
$6p_{3/2}$ & $5d_{3/2}$ & 4.062   & -0.146 & -0.878 & 0.227  & -0.145 & -0.005 & -0.001 & 0.054  & 3.167   &             &        &         \\
*$7p_{1/2}$ & $5d_{3/2}$ & -4.039  & -0.107 & 2.402  & -0.499 & 0.341  & 0.018  & 0.003  & -0.158 & -2.039  &             &        &         \\
*$7p_{3/2}$ & $5d_{3/2}$ & -1.688  & -0.048 & 1.060  & -0.220 & 0.150  & 0.007  & 0.002  & -0.069 & -0.806  &             &        &         \\[0.1cm]
$6p_{3/2}$ & $5d_{5/2}$ & 12.186  & -0.435 & -2.494 & 0.632  & -0.414 & -0.017 & -0.003 & 0.154  & 9.608   &             &        &         \\
*$7p_{3/2}$ & $5d_{5/2}$ & -5.025  & -0.141 & 3.006  & -0.612 & 0.431  & 0.024  & 0.004  & -0.201 & -2.513  &             &        &         \\
*$7p_{3/2}$ & $5d_{5/2}$ & -5.025  & -0.141 & 3.006  & -0.612 & 0.431  & 0.024  & 0.004  & -0.201 & -2.513  &             &        &       \\
\hline
\hline     
\end{tabular}
...
$\delta\Sigma^{(\infty)}$ is the all-orders correlation correction  (beyond second-order), including screening and hole-particle interaction.
Note that for the $n=7$ states (*) the correlation potential was evaluated at the energies of the $n=6$ states.
Note that calculated frequencies, not experimental values, are used (except for the $n=5,6$ states, where the $\lambda$ reproduces the experimental levels). Since lifetimes scale as $\w^3$, this is the leading source of error.
\label{tab:Cs-tau-full}}
\begin{tabular}{r  D{.}{.}{3.2} D{.}{.}{4.1}D{.}{.}{4.1} D{.}{.}{2.2}D{.}{.}{2.2}D{.}{.}{4.1} D{.}{.}{4.1} D{,}{}{4.7} D{,}{}{4.5}}
\hline
\hline   
\multicolumn{1}{c}{State}      
& \multicolumn{1}{c}{RPA}      
...
\hline   
$6p_{1/2}$ & 51.8  & -21.2  & 4.3    & 0.16  & -0.06 & -0.3   & 34.7   & 34,.934(94) & 34,.4(12) \\
$6p_{3/2}$ & 45.9  & -19.1  & 3.6    & 0.12  & 0.00  & -0.2   & 30.3   & 30,.460(38) & 30,.0(7)  \\
$5d_{3/2}$ & 227.2 & 1266.3 & -293.6 & 6.2   & 40.6  & -269.8 & 976.9  & 909,(15)    & 966,(34)  \\
$5d_{5/2}$ & 284.1 & 1834.0 & -410.6 & 10.1  & 62.2  & -415.0 & 1364.8 & 1353,(5)    & 1351,(52) \\
*$7s_{1/2}$ & 57.9  & -11.9  & 1.2    & -0.02 & 0.04  & 0.4    & 47.6   & 49,(4)      & 48,.4(2)  \\
*$7p_{1/2}$ & 186.5 & -29.5  & 15.0   & 0.91  & -0.05 & -4.6   & 168.3  & 155,(4)     & 152,(18)  \\
*$7p_{3/2}$ & 163.1 & -35.5  & 14.5   & 0.72  & 0.24  & -3.3   & 139.8  & 133,(2)     & 128,(10) \\
\hline
\hline   
\end{tabular}\\
$^\dag$Experimental results tabulated in Ref.~\cite{Safronova2016b}, except for $5d_{5/2}$ which is from Ref.~\cite{Pucher2019}.\\
