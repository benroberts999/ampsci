...
\newcommand{\threej}[6]{\small\ensuremath{\begin{pmatrix}#1&#2&#3\\#4&#5&#6\end{pmatrix}}}	% 3-j symbol
\newcommand{\sixj}[6]{\small\ensuremath{\begin{Bmatrix}#1&#2&#3\\#4&#5&#6\end{Bmatrix}}}	% 6-j symbol
...
\def\updatedDate{4 November 2020}
...
%
Here, I outline a different method that very well approximates the HF method. 
We use this only as a starting point for the HF (and TDHF) procedure, and the the final result does not depend on this potential. 
...
However, when $F_a$ is small, the exchange potential is less important (only the combination $V_xF_a$ enters the equations).
...
\end{align}
%
We don't apply the cut-off when $b=a$ in the sum, since here the cancellation is exact and there is no numerical instability.
...
%Note: when forming the correlation potential using the Feynman method, the Breit Hamiltonian is not (for now) included when calculating the Hartree-Fock Green's functions.
%This mean Breit should not be included into the Hartree-Fock equations before the (Feynman) correlation potential is formed (since the Green's function would not have the proper orthogonality conditions to the core states).
%The correlation potential should be calculated first, then once the correlation potential has been formed, it is safe to include Breit.
%It is fine, however, to include Breit when calculating the second-order correlation potential, since the Breit Hamiltonian is included into the spline basis states.
...
\begin{tabular}{l D{.}{.}{6.1} D{.}{.}{2.1}D{.}{.}{2.1} l D{.}{.}{2.1}D{.}{.}{2.1}}
\hline
\hline
        &           & \multicolumn{2}{c}{$\delta$QED}       && \multicolumn{2}{c}{$\delta$Breit}       \\
\cline{3-4}\cline{6-7}
        & \multicolumn{1}{c}{$\en~(\Sigma^{(2)})$} &  \multicolumn{1}{c}{HF}    & \multicolumn{1}{c}{$\Sigma^{(2)}$} && \multicolumn{1}{c}{HF}    & \multicolumn{1}{c}{$\Sigma^{(2)}$} \\
\hline
$6s_{1/2}$ & 32412.1   & -16.0 & -23.7& & -3.2  & 2.9   \\
$7s_{1/2}$ & 13023.0   & -4.3  & -5.2 & & -1.1  & -0.1  \\
$6p_{1/2}$ & 20537.8   & 0.8   & 1.1  & & -7.5  & -7.3  \\
$7p_{1/2}$ & 9710.0    & 0.3   & 0.4   && -2.7  & -2.6  \\
$6p_{3/2}$ & 19939.2   & 0.1   & 0.0  & & -2.9  & -0.6  \\
$7p_{3/2}$ & 9520.5    & 0.0   & 0.0   && -1.0  & -0.4  \\
\hline
\hline
\end{tabular}
\end{table}
...
\hline
\hline
         &                  &      \multicolumn{3}{c}{ $\delta$ Breit}  \\
\cline{3-5}
        & \multicolumn{1}{c}{$|E1|$} &  \multicolumn{1}{c}{HF}    & \multicolumn{1}{c}{RPA} & \multicolumn{1}{c}{RPA+$\Sigma^{(2)}$}\\
\hline
$6p_{1/2}$--$ 6s_{1/2}$ & 4.3874 &     0.0004 & 0.0000 & -0.0008   \\
$7p_{1/2} $--$ 6s_{1/2}$ & 0.3001 & 0.0018 & 0.0016 & 0.0017    \\
$6p_{3/2} $--$ 6s_{1/2} $& 6.1698 &     0.0008 & 0.0001 & -0.0009   \\
$7p_{3/2} $--$ 6s_{1/2} $& 0.6056   & 0.0006 & 0.0003 & 0.0002    \\
$7p_{1/2} $--$ 7s_{1/2} $& 10.178      & -0.0010 & -0.0011 & -0.0022    \\
$7p_{3/2} $--$ 7s_{1/2} $& 14.123      & 0.0008  & 0.0006  & 0.0004     \\
$7s_{1/2} $--$ 6p_{1/2} $& 4.2241    & 0.0046 & 0.0045 & 0.0045    \\
$7s_{1/2} $--$ 6p_{3/2} $& 6.4705  & 0.0018 & 0.0018 & 0.0010  \\
\hline
\hline
\end{tabular}
\end{table}
...
& \multicolumn{1}{c}{$\delta$QED} 
& \multicolumn{1}{c}{$\delta$Breit}
& \multicolumn{1}{c}{$\delta$SR} 
& \multicolumn{1}{c}{$\delta$Norm} 
...
\end{multline}
and the symmetric $y^k_{bd} = y^k_{db}$ ``Hartree Screening function'' is defined:
\be
y^k_{bd}(r) = \int_0^\infty \frac{r_<^k}{r_>^{k+1}}\left[f_a(r') f_b(r') + g_a(r') g_b(r')\right]\, \d r',
\ee
and the angular factor is:
\begin{align}
