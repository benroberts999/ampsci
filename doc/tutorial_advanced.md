...
For example, to run Hartree-Fock for Cs with a \f$N-1\f$ (implies Xe-like core),
...
Typically, we instead set `r0_eps`, which automatically choses `r0` such that the core density \f$\rho_l(r) = \sum_n|\phi_{nl}(r)|^2\f$ drops below given relative value \f$\rho_l(r0)/\rho_{\rm max}<{\rm r0\_{\rm eps}}\f$.
...
The worst energy comparison was for \f$5s\f$ state (parts in \f$10^5\f$), and the worst orthogonality comparison was for the \f$3s\f$ finite-difference Hartree-Fock state, and the \f$22s\f$ basis state, which were orthogonal to parts in \f$10^4\f$.
...
There is also a `ModulebriefStructureRad` module, which gives some finer control.
